...
func (c *cmdb) UpdateObjClassification(ctx context.Context, clsId string, objCls *model.ObjClassification) (interface{}, error) {
	result, err := c.db.Collection(objCls.TableName()).UpdateOne(ctx,
		bson.M{"classification_id": clsId},
		bson.M{"$set": objCls})
	if err != nil {
		return nil, err
	}
...
func (c *cmdb) CheckObjClassificationExists(ctx context.Context, clsId string) (bool, error) {
	var objCls model.ObjClassification

	_, err := c.db.Collection(objCls.TableName()).Find(ctx, bson.M{"classification_id": clsId})
	if err != nil {
		return false, err
	}
